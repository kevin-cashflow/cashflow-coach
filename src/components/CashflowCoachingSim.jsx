...
> [위 진단 근거 중 가장 인상적인 2~3개를 선별해 인용 — 구체적 숫자 포함]

[페르소나의 의미를 1~2문장으로 풀어서 설명]

[다음 시도 권장 사항을 1~2문장 + 구체적 행동 1가지]

⚠️ 위 형식은 변형하지 마세요. 페르소나 이름과 분류, 진단 근거 숫자는 위 데이터를 그대로 사용하세요.
⚠️ 페르소나 섹션은 본문 코칭 메시지 앞쪽 (인사말 직후) 또는 뒤쪽 (마무리 전) 중 자연스러운 위치에 배치하세요.

추가로:
- 본문 다른 부분에서도 페르소나 이름을 1~2회 더 자연스럽게 언급하면 좋습니다
  (예: "당신처럼 ${info.name || "○○형"}인 분들은 보통...")
- 진단 근거의 숫자를 본문 코칭과 연결하세요 (예: "활용률 78%라는 게임 데이터가 보여주듯...")
═══════════════════════════════════════════════════
`;
}
...
${topEvidence || "(진단 근거 미확보)"}

### 💡 의미

${meaningLine || diagnosisLine || "(진단 내용 미확보)"}

### 🌱 다음 시도

${nextStepLine || "(권장 사항 미확보)"}`;
}
